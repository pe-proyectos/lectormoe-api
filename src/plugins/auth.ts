...
// Same as loggedUserOnly but without the x-organization requirement.
// For global / cross-organization endpoints (e.g. joint chapter history) where the caller
// is identified by JWT alone and the resource isn't scoped to one org.
export const loggedUserOnlyGlobal = () => new Elysia()
    .use(
        jwt({
            name: 'jwt',
            secret: Bun.env.JWT_SECRET as string,
    ...
    .derive({ as: 'scoped' }, async ({ jwt, request: { headers } }) => {
        ...
        const user = await checkToken(null, token);
        if (!user) {
            throw new Error('No autorizado, usuario no encontrado.');
        }
