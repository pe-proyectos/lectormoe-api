...
/**
 * Cambia una suscripcion existente a otro plan (subir o bajar de nivel).
 *
 * PayPal no cobra dos suscripciones ni un prorrateo: el precio nuevo se aplica
 * desde el siguiente ciclo, asi que el lector solo paga la diferencia hacia
 * adelante. Si el cambio sube el precio, PayPal exige que el comprador lo
 * apruebe: en ese caso la respuesta trae un enlace `approve` al que hay que
 * mandarlo, y al volver se confirma el cambio.
 */
export async function reviseSubscription(
  ...
  returnUrl: string,
  cancelUrl: string
): Promise<{ approveUrl: string | null; raw: any }> {
  const token = await getAccessToken();

  const response = await fetch(`${environment.url}/billing/subscriptions/${paypalSubscriptionId}/revise`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
  ...
  if (!response.ok) {
    ...
    throw new Error("No se pudo cambiar el plan en PayPal.");
  }

  const raw = await response.json();
  const approveUrl = Array.isArray(raw?.links)
    ? raw.links.find((l: any) => l.rel === "approve")?.href ?? null
    : null;
  return { approveUrl, raw };
}
