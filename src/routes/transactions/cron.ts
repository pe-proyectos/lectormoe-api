...
async function calculateTransactions() {
  try {
    console.log(
      '🔄 Iniciando sincronización de transacciones de suscripciones...'
    )
    ...
    for (const organization of organizations) {
      try {
        ...
        if (subscriptions.length === 0) {
          console.log(
            `  No hay suscripciones activas para la organización ${organization.slug}`
          )
          continue
        }
        ...
        for (const subscription of subscriptions) {
          try {
            if (!subscription.paypalSubscriptionId) {
              console.log(
                `  ⚠️ Suscripción ${subscription.id} no tiene paypalSubscriptionId, saltando...`
              )
              continue
            }
            ...
            if (!Array.isArray(transactions) || transactions.length === 0) {
              console.log(
                `    No se encontraron transacciones para la suscripción ${subscription.id}`
              )
              continue
            }
                ...
                if (transactionExists) {
                  await prisma.organizationTransaction.update({
                    ...
                    data: {
                      ...
                      amount: finalAmount,
                      currency:
                        transaction.amount_with_breakdown?.net_amount
                          ?.currency_code || 'USD',
                      type: 'EARNING',
                  ...
                  console.log(
                    `    ✅ Transacción ${transaction.id} actualizada`
                  )
                } else {
                  ...
                  console.log(
                    `    ✅ Transacción ${transaction.id} creada y vinculada a suscripción ${subscription.id}`
                  )
                }
              } catch (error: any) {
                totalErrors++
                console.error(
                  `    ❌ Error procesando transacción ${transaction?.id || 'unknown'}:`,
                  error?.message || error
                )
              }
            }
            ...
            totalErrors++
            console.error(
              `  ❌ Error procesando suscripción ${subscription?.id || 'unknown'}:`,
              error?.message || error
            )
          }
        }
        ...
        totalErrors++
        console.error(
          `❌ Error procesando organización ${organization?.id || 'unknown'}:`,
          error?.message || error
        )
      }
    }
    ...
    console.error(
      '❌ Error crítico en calculateTransactions:',
      error?.message || error
    )
  }
}
...
export const router = () =>
  new Elysia().use(
    cron({
      ...
      run: wrapCron('sync-subscription-transactions', calculateTransactions)
    })
  )
