...
import { BASE_URL, sendBatchEmails } from '../../services/email'
import {
  canSendEmail,
  getUnsubscribeUrl
...
async function processDailyDigest() {
  try {
    ...
    console.log(
      `[Email Cron] Daily digest complete. Queued ${emailsToSend.length} emails.`
    )
  } catch (error) {
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        const userRank = readerCounts.findIndex((r) => r.userId === user.id) + 1
        const percentile =
  ...
  try {
    ...
    if (allOrgViews.length > 0) {
      ...
      for (const v of allOrgViews) {
        ...
        if (orgId)
          orgViewMap.set(orgId, (orgViewMap.get(orgId) || 0) + v._count)
      }
    }
    ...
    console.log(
      `[Email Cron] Weekly org reports complete. Queued ${allEmailsToSend.length} emails.`
    )
  } catch (error) {
...
async function processReEngagement() {
  try {
    ...
    console.log(
      `[Email Cron] Re-engagement complete. Queued ${emailsToSend.length} emails.`
    )
  } catch (error) {
...
async function processReadingStreaks() {
  try {
    ...
    console.log(
      `[Email Cron] Reading streaks complete. Queued ${emailsToSend.length} milestone emails.`
    )
  } catch (error) {
...
async function processMonthlyRecap() {
  try {
      ...
      try {
        ...
        const history = await prisma.userChapterHistory.findMany({
          ...
          include: {
            chapter: {
              include: {
                mangaCustom: { include: { genres: { select: { name: true } } } }
              }
            }
          }
        })
        ...
        const mangasRead = new Set(history.map((h) => h.chapter.mangaCustomId))
          .size
    ...
    console.log(
      `[Email Cron] Monthly recap complete. Queued ${emailsToSend.length} emails.`
    )
  } catch (error) {
...
async function processTopReaderLeaderboard() {
  try {
    ...
    console.log(
      `[Email Cron] Top reader leaderboard complete. Queued ${allEmailsToSend.length} emails.`
    )
  } catch (error) {
    console.error(
...
async function processReadingAchievements() {
  try {
    ...
    console.log(
      `[Email Cron] Achievements check complete. Queued ${emailsToSend.length} emails.`
    )
  } catch (error) {
  ...
  new Elysia()
    ...
    .use(
      cron({
        ...
        run: wrapCron('email-reading-streaks', processReadingStreaks)
      })
    )
    .use(
      cron({
        ...
        run: wrapCron('email-monthly-recap', processMonthlyRecap)
      })
    )
    ...
    .use(
      cron({
        ...
        run: wrapCron('email-reading-achievements', processReadingAchievements)
      })
    )
