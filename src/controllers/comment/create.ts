...
import type { CreateCommentRequest } from '../../types/comment/create'
...
export const createComment = async (
  ...
  if (activeBan) {
    const banLabel =
      activeBan.type === BanType.TEMPORARY
        ? 'temporalmente baneado'
        : activeBan.type === BanType.RESTRICTED
          ? 'restringido'
          : 'baneado permanentemente'
    throw new Error(
      `No puedes comentar: estás ${banLabel} en este scan.${activeBan.reason ? ` Motivo: ${activeBan.reason}` : ''}`
    )
  }

  const parentId = params.parentId ? parseInt(params.parentId.toString()) : null
  if (parentId) {
    const parent = await prisma.comment.findFirst({
      where: { id: parentId, organizationId, identifier: params.identifier },
      select: { id: true, deletedAt: true, hiddenAt: true }
    })
    if (!parent)
      throw new Error('No se encontró el comentario al que respondes.')
    if (parent.deletedAt || parent.hiddenAt)
      throw new Error('Ese comentario ya no está disponible.')
  }
